  ...
  <main>
    ...
    <div class="form-container">
      ...
      <input
        ...
        v-model="dogBreed"
      />
      <button id="add-btn-form" @click="saveDogForm">
        Add Card (Form values)
      </button>
    </div>
    <!--
        ...
        -->
    <div class="form-container">
      <label for="age-filter">Age filter</label
      ><input
        id="age-filter"
        type="text"
...
const filtered = computed(() => {
  ...
  if (age.value == 0 || age.value == "") return dogs.value;

  const arr = [];

  // Filtering the list
  for (let dog of dogs.value) {
    if (dog.age < age.value) arr.push(dog);
  }
